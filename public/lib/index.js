...
lib.factory('userManager', function ($http) {
    ...
    userManager.login = function (username, password) {
        return $http.get('http://shutapp.nu:3000/login/' + username + '/' + password);
    };
    userManager.logout = function () {
        return $http.get('http://shutapp.nu:3000/logout');
    };
    userManager.signupuser = function (signupCredentials) {
        return $http.post('http://shutapp.nu:3000/signup', signupCredentials);
    };
...
lib.factory('messageManager', function ($http) {
    ...
    messageManager.getChatrooms = function () {
        return $http.get('http://shutapp.nu:3000/chatrooms');
    };
    ...
    messageManager.getMessages = function (chatroomId) {
        return $http.get('http://shutapp.nu:3000/messages?chatroom=' + chatroomId);
    };
    messageManager.getPrivateMessages = function (user, otheruser) {
        return $http.get('http://shutapp.nu:3000/messages?user=' + user + '&otheruser=' + otheruser);
    };
    messageManager.getConversations = function (userId) {
        return $http.get('http://shutapp.nu:3000/conversations?userid=' + userId);
    };
    messageManager.postMessages = function (newMessage) {
        return $http.post('http://shutapp.nu:3000/messages', newMessage);
    };
